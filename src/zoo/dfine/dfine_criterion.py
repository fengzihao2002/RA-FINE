...
class DFINECriterion(nn.Module):
    ...
    def __init__(
        ...
        self.use_nwd_aux = bool(use_nwd_aux)
        self.nwd_constant = float(nwd_constant)
        self.use_ma_ddf = bool(use_ma_ddf)
        self.ma_balance_warmup_epochs = int(ma_balance_warmup_epochs)
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, boxes_weight=None):
        ...
        if self.use_nwd_aux and "loss_nwd" in self.weight_dict:
            # Gaussian-Wasserstein distance for normalized cx,cy,w,h boxes.
            center = (src_boxes[:, :2] - target_boxes[:, :2]).pow(2).sum(-1)
            size = ((src_boxes[:, 2:] - target_boxes[:, 2:]) * 0.5).pow(2).sum(-1)
            wasserstein = torch.sqrt((center + size).clamp_min(1e-12))
            similarity = torch.exp(-wasserstein / max(self.nwd_constant, 1e-6))
            loss_nwd = 1.0 - similarity
            loss_nwd = loss_nwd if boxes_weight is None else loss_nwd * boxes_weight
        ...
        if "pred_corners" in outputs:
            ...
            if "teacher_corners" in outputs:
                ...
                if torch.equal(pred_corners, target_corners):
                    ...
                    teacher_confidence = outputs.get("teacher_confidence")
                    if self.use_ma_ddf and teacher_confidence is not None:
                        weight_targets_local = weight_targets_local * teacher_confidence.detach().to(
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        extra_losses = self._full_regularizer_losses(
            outputs, epoch=kwargs.get("epoch", 0)
        )
        extra_losses = {
    ...
    def _full_regularizer_losses(self, outputs, epoch=0):
        diagnostics = outputs.get("full_diagnostics")
        if not isinstance(diagnostics, dict):
            return {}

        losses = {}
        upstream = diagnostics.get("upstream") or {}
        fs_diag = upstream.get("fs") if isinstance(upstream, dict) else None
        if isinstance(fs_diag, dict):
            if "offset_abs" in fs_diag:
                losses["loss_fs_offset"] = fs_diag["offset_abs"]
            if "offset_smooth" in fs_diag:
                losses["loss_fs_smooth"] = fs_diag["offset_smooth"]

        ma_diag = diagnostics.get("ma")
        if isinstance(ma_diag, dict) and ma_diag.get("gates") is not None:
            gates = ma_diag["gates"]
            mean_gate = gates.mean(dim=(0, 1, 2, 3))
            if self.ma_balance_warmup_epochs > 0:
                decay = max(
                    0.0,
                    1.0 - float(epoch) / float(self.ma_balance_warmup_epochs),
                )
            else:
                decay = 1.0
            losses["loss_ma_balance"] = decay * ((mean_gate - 0.5) ** 2).sum()

            rgb_delta = ma_diag.get("rgb_delta")
            ir_delta = ma_diag.get("ir_delta")
            gamma = ma_diag.get("gamma")
            if rgb_delta is not None and ir_delta is not None and gamma is not None:
                gamma_view = gamma.reshape(-1, 1, 1, 1, 1)
                losses["loss_ma_residual"] = (
                    (gamma_view * rgb_delta).abs().mean()
                    + (gamma_view * ir_delta).abs().mean()
                )
        return losses
