...
from .multimodal_full import ModalSideProjection
...
class HybridEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.use_modal_side_paths = bool(use_modal_side_paths)
        ...
        # Lightweight modality-specific side paths used only by MA-FDR.
        # They do not replace the main fused encoder path.
        if self.use_modal_side_paths:
            self.modal_rgb_proj = nn.ModuleList(
                [ModalSideProjection(c, hidden_dim) for c in in_channels]
            )
            self.modal_ir_proj = nn.ModuleList(
                [ModalSideProjection(c, hidden_dim) for c in in_channels]
            )
        else:
            self.modal_rgb_proj = nn.ModuleList()
    ...
    def forward(self, feats):
        # Preserve compatibility with original single-list calls.
        if not isinstance(feats, dict):
            return self._forward_fused(feats)

        fused = feats["fused"]
        outs = self._forward_fused(fused)
        use_ma_fdr = bool(feats.get("use_ma_fdr", False))

        if use_ma_fdr:
            if not self.use_modal_side_paths:
                raise RuntimeError(
                    "MA-FDR is enabled but HybridEncoder.use_modal_side_paths is False"
                )
            rgb_feats = feats["rgb"]
            ir_feats = feats["ir"]
            reliability = feats["reliability"]
            if not (
                len(rgb_feats)
                == len(ir_feats)
                == len(reliability)
                ...
                raise RuntimeError("Modal side path scale count mismatch")

            modal_rgb = [
                self.modal_rgb_proj[i](rgb_feats[i]) for i in range(len(rgb_feats))
            ]
            modal_ir = [
                self.modal_ir_proj[i](ir_feats[i]) for i in range(len(ir_feats))
            ]
            reliability_out = [
                F.interpolate(
                    reliability[i],
                    size=modal_rgb[i].shape[-2:],
                    mode="bilinear",
                    align_corners=False,
                )
